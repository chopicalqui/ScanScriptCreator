...
set domain=53
set tftp=69
set http=80,443,1080,5357,5800,8014,8080,8081,8082,8088,8443,8888,9090,10000
set pop=109,110
set rpcbind=111
...
set tcp_ports=%telnet%,%ftp%,%ssh%,%msprc%,%smtp%,%domain%,%tftp%,%http%,%pop%,%rpcbind%,%adds%,%sftp%,%snmp%,%smb%,%vpn%,%imap%,%rlogin%,%rmi%,%mssql%,%oracle%,%nfs%,%mysql%,%rdp%,%postgresql%,%x11%,%sip%,%vnc%,%mongodb%,%printer%,%winrm%
set udp_ports=%dhcp%,%tftp%,%rpcbind%,%ntp%,%snmp%,%vpn%,%nfs%,%domain%,%mssql%,%vnc%,%x11%


rem Initialization of Default NSE Scripts List
set tcp_scripts= --script http-title,nfs-showmount,http-methods,smb-enum-*,vnc-info,krb5-enum-users,vnc-title,http-robots.txt,x11-access,ms-sql-info,pop3-capabilities,ssl-cert,ftp-anon,imap-capabilities,smtp-commands,rmi-dumpregistry,mongodb-info,mongodb-databases
set udp_scripts= --script ms-sql-info,x11-access,dns-service-discovery,ntp-info


rem Initialization of Nmap Options
set nmap_options=-v --stats-every 10 --reason -Pn -sV --max-retries 1 --min-hostgroup 64 --traceroute
